...
static bool exmdbc_settings_check(void *_set, pool_t pool, const char **error_r);

static const struct setting_define exmdbc_setting_defines[] = {
...
static const struct imapc_settings exmdbc_default_settings = {
...
static const struct setting_keyvalue exmdbc_default_settings_keyvalue[] = {
...
bool exmdbc_settings_check(timer_t set, pool_t pool, const char ** error_r);

const struct setting_parser_info exmdbc_setting_parser_info = {
	.name = "exmdbc_imapc",

	.defines = exmdbc_setting_defines,
	.defaults = &exmdbc_default_settings,
	.default_settings = exmdbc_default_settings_keyvalue,

	.struct_size = sizeof(struct imapc_settings),
	.pool_offset1 = 1 + offsetof(struct imapc_settings, pool),

	.check_func = exmdbc_settings_check
};

/* <settings checks> */
struct exmdbc_feature_list {
	const char *name;
	enum imapc_features num;
};

static const struct exmdbc_feature_list exmdbc_feature_list[] = {
...
static int
exmdbc_settings_parse_throttle(struct imapc_settings *set,
...
static int
exmdbc_settings_parse_features(struct imapc_settings *set,
			      const char **error_r)
{
	enum imapc_features features = 0;
	const struct exmdbc_feature_list *list;
	const char *const *str, *value;

	str = settings_boollist_get(&set->imapc_features);
	for (; *str != NULL; str++) {
		list = exmdbc_feature_list;
		...
		if (str_begins_icase(*str, "throttle:", &value)) {
			if (exmdbc_settings_parse_throttle(set, value, error_r) < 0)
				return -1;
			continue;
		}
		if (list->name == NULL) {
...
static bool exmdbc_settings_check(void *_set, pool_t pool ATTR_UNUSED,
				 const char **error_r)
{
	...
	if (exmdbc_settings_parse_features(set, error_r) < 0)
		return FALSE;
	return TRUE;
}
